...
wdk.namespace("window.wdk.parameterHandlers", function(ns, $) {
  ...
  function initDependentParamHandlers(element) {
    ...
    Object.keys(dependentParamsMap).forEach(function(dependedName) {
      ...
      var previousValue = dependedParam.find(':text, textarea, :checked, select').val(); // EGA  -  radio buttons the checked is a prop, not the attribute, so "checked=true does not get updated when different buttons selected"
      dependedValuesMap[dependedName] = typeof previousValue === 'undefined' ? (dependedParam.prop('checked') ? dependedParam.val() : previousValue) : previousValue;

      var handleChange = function handleChange(e) {
        var newValue = $(e.target).find(':text, textarea, :checked, select').val();
        if (typeof newValue === 'undefined') {
          newValue =  $(e.target).prop('checked') ? $(e.target).val(): newValue; // EGA  -  radio buttons the checked is a prop, not the attribute, so "checked=true does not get updated when different buttons selected"
        }
