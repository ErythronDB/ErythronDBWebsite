  ...
  RouteComponentProps<any>> = ({
    ...
    useEffect(() => {
      if (searchQueue.length && !searchInProgress.current) {
        ...
        wdkService
          ._fetchJson<SearchResult[]>("get", `/lookup/gene?query=${term}`)
...
const _buildResultDisplay = (result: SearchResult, searchTerm: string) => {
  return result.display === result.matched_term ? (
    <span>{safeHtml(result.display)}&nbsp;
    <small>
        <em>{toProperCase(result.record_type)}</em>
      </small>
    </span>
  ) : (
      <span>
      ...
      <small>
          <em>{toProperCase(result.record_type)} // {_truncateMatch(result.matched_term, searchTerm)}</em>
        </small>
      </span>
    );
};
