...
function node_popup_simple(event, node, label, referer) {
    ...
    var show_item_info_URL = js_comm_globals.web_cgi_dir + "show_item_info.pl?";
...
function loadFloatingProteinWindowCoor(node, action_option, expected_width, expected_height, payload_string, label, xPos, yPos) {
    ...
    //show_item_info_URL += "&add_nodes=" + add_nodes.join("%20");
    ...
    if (payload_string !== "") {
        show_item_info_URL += "&payload_text=" + payload_string;
    }
...
function loadFloatingProteinWindow(event, node, action_option, width, height, payload_string, label, consider_offset_correction) {
    ...
    return loadFloatingProteinWindowCoor(node, action_option, width, height, payload_string, label, event.clientX + xPositionOffset, event.clientY + yPositionOffset);
}
...
function loadFloatingInteractionWindowCoor(node1, node2, label, xPos, yPos) {
    ...
    var show_edge_popup_URL = js_comm_globals.web_cgi_dir + "/show_edge_popup.pl?";
...
function getSTRING(root_url, params) {
    ...
    // Promise wrapper added by EGA 1/2020
    ...
    return new Promise(function (resolve, reject) { 
        xhr.onreadystatechange = function () {
            ...
            if (xhr.status >= 200 && xhr.status < 300) {
                ...
                resolve(xhr); // not sure if this is right
            }
            else {
                reject({
                    status: xhr.status,
...
function setMovableNode(obj) {
    ...
    obj.onmouseupCallback = function (event) {
        obj.original_event_target = event.currentTarget;
        return (moveObjectMUp.apply(obj, arguments));
    }
    obj.onmousemoveCallback = function (event) {
        obj.original_event_target = event.currentTarget;
        return (moveObjectMMove.apply(obj, arguments));
    }
    obj.ontouchendCallback = function (event) {
        obj.original_event_target = event.currentTarget;
        return (moveObjectTUp.apply(obj, arguments));
    }
    obj.ontouchmoveCallback = function (event) {
        obj.original_event_target = event.currentTarget;
        return (moveObjectTMove.apply(obj, arguments));
    }
}
...
function update_network_coordinates_at_server(taskId, timeout) {
    ...
    if (typeof (Worker) !== "undefined") {
        ...
        xmlhttp.open("POST", js_comm_globals.web_cgi_dir + "/update_network_coordinates.pl", true);
...
function handle_network_node_click(event) {
    ...
    var payload_text = this.getAttribute("data-payload_text");
    var safe_div_label = this.getAttribute("data-safe_div_label");

    hideFloatingDiv("fpWindowDiv"); // just in case it is showing somewhere else.

    loadFloatingProteinWindow(event, node_id, action_option, expected_popup_width, expected_popup_height, payload_text, safe_div_label, true);
...
function init_network_interactive_functionalities(event) {
    ...
    if (edge_elements !== null) {
        for_each_node(edge_elements, function register_edge_with_its_nodes(edge) {
            ...
            lx = Math.abs(lx);
            ly = Math.abs(ly);
            if (lj.n1.ix > lj.n2.ix) {
                lx *= -1.0;
            }
            if (lj.n1.iy > lj.n2.iy) {
                ly *= -1.0;
            }
            ...
            lj.x1 -= lj.n1.x;
            lj.y1 -= lj.n1.y;
            tmpx = lj.x1 * lx + lj.y1 * ly;
            tmpy = lj.x1 * px + lj.y1 * py; // projection of line vector on line unit vector and perp unit vector
            lj.x1 = tmpx;
            lj.y1 = tmpy;
            lj.x2 -= lj.n2.x;
            lj.y2 -= lj.n2.y;
            tmpx = lj.x2 * lx + lj.y2 * ly;
            tmpy = lj.x2 * px + lj.y2 * py; // projection of line vector on line unit vector and perp unit vector
            lj.x2 = tmpx;
            lj.y2 = tmpy;
...
//--------------------------------------------------------------
// Author: Andrea Franceschini, Christian von Mering
// Last Update: February 2018
// Version: 0.5
//--------------------------------------------------------------

//************************************************************************************
// Copyright (C) 2009, Andrea Franceschini, Massimo Beatini, Nicolas
//
// This script has been built adapting the code from Massimo Beatini
// and Nicolas - http://www.javascript-page.com
//************************************************************************************
...
function displayFloatingDivImp(divId, title, width, height, left, top, headerColor, hideDelay, recenter, previous_width) {
    ...
    if (document.getElementById(divId).style.visibility != "visible") {
        ...
        if (recenter) {
            ...
            if (new_left - scrOfX + new_width > windowWidth - 20) {
                new_left = scrOfX + windowWidth - (20 + new_width);
            }
            if (new_left < 20 + scrOfX) {
                new_left = 20 + scrOfX;
            }
...
function displayFloatingDiv(divId, title, expected_width, expected_height, width, height, left, top, headerColor, hideDelay, url, loadingImgUrl) {
    ...
    if (expected_width != null && expected_width > 0) {
        waiting_width = expected_width;
    }
    var waiting_height = 350;
    if (expected_height != null && expected_height > 0) {
        waiting_height = expected_height;
    }
...
function registerMovableDivTitleBar(elm) {
    ...
    elm.onmousemoveCallback = function (event) {
        elm.original_event_target = event.currentTarget;
        return (movable_div_MMove_handler.apply(elm, arguments));
    }
    elm.ontouchmoveCallback = function (event) {
        elm.original_event_target = event.currentTarget;
        return (movable_div_TMove_handler.apply(elm, arguments));
    }
    elm.onmouseupCallback = function (event) {
        elm.original_event_target = event.currentTarget;
        return (movable_div_MUp_handler.apply(elm, arguments));
    }
    elm.ontouchendCallback = function (event) {
        elm.original_event_target = event.currentTarget;
        return (movable_div_TUp_handler.apply(elm, arguments));
    }
}
